...
export class SearchResult extends HTMLElement {
  ...
  render() {
    this.results.forEach((recipe) => {
      this.querySelector("section").insertAdjacentHTML(
        "afterBegin",
        `
      ...
      recipe.ingredients.forEach((ingredient) =>
        this.querySelector(".ingredients").insertAdjacentHTML(
          "beforeend",
  ...
  observer() {
              ...
              if (addedNode.classList.contains("ingredient")) {
                this.ingredients = this.ingredients.filter(
                  (ingredient) =>
                    ...
                    addedNode.innerHTML.toLowerCase()
                );
    ...
    observer.observe(document.querySelector(".searchParams"), {
      childList: true,
    });
  }
  ...
  querySearch() {
    // clean old results
    this.querySelectorAll("article").forEach((element) => {
      element.remove();
    });
